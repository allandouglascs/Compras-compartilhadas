const STORAGE_KEY = 'finance-control-app:v1';
...
function hydrateState() {
  ...
  if (stored) {
    try {
      ...
      if (parsed.settings?.theme) state.settings.theme = parsed.settings.theme;
      if (parsed.ui) state.ui = { ...state.ui, ...parsed.ui };
    } catch {
      showToast('Não foi possível ler os dados salvos. Um novo estado foi iniciado.', 'error');
    }
  }
...
function bindEvents() {
  ...
  el.monthSelect.addEventListener('change', () => { state.ui.selectedMonth = Number(el.monthSelect.value); persist(); renderAll(); });
  el.yearSelect.addEventListener('change', () => { state.ui.selectedYear = Number(el.yearSelect.value); persist(); renderAll(); });
  el.tabLinks.forEach(button => button.addEventListener('click', () => switchSection(button.dataset.section)));
  ...
  el.filterType.addEventListener('change', () => { state.ui.filterType = el.filterType.value; persist(); renderTable(); });
  el.filterNature.addEventListener('change', () => { state.ui.filterNature = el.filterNature.value; persist(); renderTable(); });
  el.searchInput.addEventListener('input', () => { state.ui.search = el.searchInput.value.trim().toLowerCase(); persist(); renderTable(); });
  el.sortButtons.forEach(button => button.addEventListener('click', () => changeSort(button.dataset.sort)));
  ...
  el.topLimitSelect.addEventListener('change', () => {
...
function buildPeriodFilters() {
  el.monthSelect.innerHTML = MONTHS.map((month, index) => `<option value="${index}">${month}</option>`).join('');
  const years = getAvailableYears();
  el.yearSelect.innerHTML = years.map(year => `<option value="${year}">${year}</option>`).join('');
...
function getAvailableYears() {
  ...
  state.transactions.forEach(item => years.add(new Date(item.date).getFullYear()));
  return [...years].sort((a, b) => b - a);
}

function switchSection(sectionId) {
  el.sections.forEach(section => section.classList.toggle('active', section.id === sectionId));
  el.tabLinks.forEach(button => button.classList.toggle('active', button.dataset.section === sectionId));
}
...
function handleFormSubmit(event) {
  ...
  if (payload.id) {
    state.transactions = state.transactions.map(item => item.id === payload.id ? payload : item);
...
function renderDashboard() {
  ...
  el.averageTicket.textContent = formatCurrency(transactions.length ? (transactions.reduce((sum, item) => sum + item.amount, 0) / transactions.length) : 0);
  el.largestExpense.textContent = formatCurrency(Math.max(0, ...transactions.filter(item => item.type === 'custo').map(item => item.amount)));
  el.largestIncome.textContent = formatCurrency(Math.max(0, ...transactions.filter(item => item.type === 'ganho').map(item => item.amount)));
...
function renderCategoryHighlights(transactions) {
  ...
  const entries = Object.entries(expensesByCategory).sort((a, b) => b[1] - a[1]).slice(0, state.ui.topLimit);
  if (!entries.length) {
  ...
  el.categorySpendingList.innerHTML = entries.map(([category, value]) => {
    ...
    return `
      <div class="category-item">
        <div class="row"><strong>${escapeHtml(category)}</strong><span>${formatCurrency(value)}</span></div>
        <div class="progress-track"><div class="progress-bar" style="width:${Math.max(percent, 6)}%"></div></div>
      </div>
    `;
  }).join('');
}
...
function renderTable() {
  ...
  el.tableBody.querySelectorAll('[data-edit]').forEach(button => button.addEventListener('click', () => editTransaction(button.dataset.edit)));
  el.tableBody.querySelectorAll('[data-delete]').forEach(button => button.addEventListener('click', () => confirmDeletion(button.dataset.delete)));
}
...
function getFilteredTransactions() {
  ...
  return filtered.sort((a, b) => {
    const left = a[state.ui.sortField];
    const right = b[state.ui.sortField];
    if (state.ui.sortField === 'date') {
      return (new Date(left) - new Date(right)) * direction;
    }
    if (state.ui.sortField === 'amount') {
...
function renderMonthlyComparison() {
  ...
  const totals = months.map(({ year, monthIndex, label }) => {
    ...
    return { label, balance: summary.balance, income: summary.totalIncome, expense: summary.totalExpense };
  });
...
function updateCategorySuggestions() {
  ...
  el.categorySuggestions.innerHTML = [...dynamic].sort((a, b) => a.localeCompare(b, 'pt-BR')).map(item => `<option value="${item}"></option>`).join('');
}
...
function exportCsv() {
  ...
  const csv = [headers.join(';')].concat(rows.map(item => [
  ...
  downloadFile(csv, `lancamentos-${state.ui.selectedYear}-${String(state.ui.selectedMonth + 1).padStart(2, '0')}.csv`, 'text/csv;charset=utf-8;');
}
...
async function restoreBackup(event) {
  ...
  try {
    ...
    if (!Array.isArray(data.transactions)) throw new Error('Estrutura inválida.');
...
function openConfirmModal({ message, confirmText = 'Confirmar', onConfirm }) {
  ...
  fragment.getElementById?.('confirmModalMessage');
  ...
  fragment.querySelector('[data-modal-confirm]').addEventListener('click', () => {
    closeModal();
    onConfirm();
  });
...
function showToast(message, type = 'success') {
  ...
  window.setTimeout(() => {
    if (el.toastRoot.contains(toast)) el.toastRoot.removeChild(toast);
  }, 2600);
}
...
function capitalizeWords(text) {
  return text.replace(/\s+/g, ' ').split(' ').filter(Boolean).map(part => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(' ');
}
